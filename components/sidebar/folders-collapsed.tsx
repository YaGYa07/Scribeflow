...
export function FoldersCollapsed() {
  ...
  function openRenameDialog(type: "folder" | "file", id: string, title: string) {
  ...
  async function submitRename() {
    ...
    if (title.length < 3) {
      toast.warning(
        `${renameTarget.type === "folder" ? "Folder" : "File"} name must be at least 3 characters long.`
      );
      return;
    }
    ...
    if (renameTarget.type === "folder") {
      ...
      const updatedFolder: Folder = { ...folder, title };
      updateFolder(updatedFolder);
      await toast.promise(updateFolderInDb(updatedFolder), {
        loading: "Renaming folder...",
        success: "Folder renamed.",
        error: "Something went wrong! Unable to rename folder.",
      });
    } else {
                  ...
                  <div>
                    ...
                    <Tooltip delayDuration={0}>
                      <TooltipTrigger asChild>
                        <Button
                          variant="ghost"
                          onClick={() => openRenameDialog("folder", id!, title)}
                          className="size-7 p-0 text-muted-foreground"
                        >
                          ...
                          <Tooltip delayDuration={0}>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                variant="ghost"
                                onClick={() =>
                                  openRenameDialog("file", id!, title)
                                }
